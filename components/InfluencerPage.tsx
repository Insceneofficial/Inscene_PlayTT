      ...
      {!isEnded && (
        <>
          ...
          <div className="absolute right-4 bottom-20 flex flex-col items-end gap-3 z-[100] pointer-events-auto">
            ...
            {/* Chat Button - Always Visible with Glass Morphism */}
            {influencerTriggers.length > 0 && (
              <div className="flex flex-col gap-2.5">
                {influencerTriggers.slice(0, 1).map((t: any, idx: number) => (
                  <button
                    key={idx}
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    ...
                    <div className="flex flex-col items-start min-w-0 relative z-10">
                      <span className="text-[11px] font-black uppercase tracking-wider text-white drop-shadow-md">Chat with {influencerName}</span>
      ...
      <main className="pt-32 pb-20 px-6 max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <button
              onClick={async () => {
                // Get the first episode's trigger for this influencer, or use default
                const firstEpisode = influencerEpisodes[0];
                const firstTrigger = firstEpisode?.triggers?.find((t: any) => t.char === influencer.name);
                
                await handleChatInit({
                  char: influencer.name,
                  intro: firstTrigger?.intro || influencer.greeting,
                  hook: firstTrigger?.hook || `Starting a conversation with ${influencer.name}`,
                  isFromHistory: false,
                  isWhatsApp: true,
                  entryPoint: 'influencer_page_header',
                  seriesId: series.id,
                  seriesTitle: series.title,
