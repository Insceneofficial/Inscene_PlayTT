...
const ReelItem: React.FC<{
  ...
  const swipeStartTime = React.useRef<number | null>(null);
  const [showOptionsOverlay, setShowOptionsOverlay] = useState(false);
  const [isSlidingDown, setIsSlidingDown] = useState(false);
  const hasTriggeredSlideRef = React.useRef(false);
    ...
    >
      <video
        ...
        onEnded={() => {
          ...
          setIsEnded(true);
          // Also trigger slide-down animation when video ends (unless it's episode 1 with path choice)
          if (!hasTriggeredSlideRef.current && isActive && !(episode.id === 1 && onShowPathChoice)) {
            hasTriggeredSlideRef.current = true;
            setIsSlidingDown(true);
            setTimeout(() => {
              onNextEpisode();
